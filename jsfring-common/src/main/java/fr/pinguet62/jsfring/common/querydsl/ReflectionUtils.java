...
public class ReflectionUtils {
    ...
    public <T> EntityPath<T> getDefaultMetaObject(Class<T> entityType) {
        ...
        metaObjectTypeName = new StringBuffer(metaObjectTypeName).insert(metaObjectTypeName.lastIndexOf('.') + 1, 'Q').toString();
        ...
        // Field
        List<Field> fields = asList(metaObjectType.getDeclaredFields())
                .stream()
                .filter(/* Static */attr -> isStatic(attr.getModifiers()))
                .filter(/* Same type */attr -> attr.getType().equals(metaObjectType))
                .collect(toList());
        if (fields.isEmpty())
